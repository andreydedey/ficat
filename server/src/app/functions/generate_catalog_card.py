...
def generate_catalog_card(data: CatalogCard):
    ...
    pdf.cell(
        page_width,
        10,
        txt="Dados Internacionais de Catalogação na Publicação (CIP) de acordo com ISBD",
        ln=1,
        align="C",
    )
    pdf.cell(
        page_width,
        10,
        txt="Sistema de Bibliotecas da Universidade Federal do Pará",
        ln=1,
        align="C",
    )
    pdf.cell(
    ...
    pdf.cell(
        page_width,
        10,
        txt=f"{data.sobrenome_autor.upper()}, {data.nome_autor.capitalize()}",
        ln=1,
    )
    pdf.cell(
        page_width,
    ...
    pdf.cell(
        page_width, 10, txt=f"1. {', '.join(data.palavras_chave)}. I. Título.", ln=1
    )
